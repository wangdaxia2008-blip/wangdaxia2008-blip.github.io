...
(function(){
  ...
  function typeIntro(totalMs = 5000){
    ...
    const t = setInterval(()=>{
      ...
      if(i >= chars.length){
        clearInterval(t);
        // after finishing, show start circle with small delay
        setTimeout(()=> startBtn.classList.remove('hidden'), 300);
      }
    }, perChar);
  }
  ...
  // loadWords supports loading from a filename relative to ./data/
  function loadWords(filename = 'word1.json'){
